...
function ListResulltAdd({ IdPrub,NombrePrub,ListadoSeguimientos,TipoTabla }) {
    ...
    <>
      {ListadoSeguimientos.length > 0
        ? ListadoSeguimientos.map((data, index) => (
            <div key={index} id={'SeguRest' + data.CODIGO_RESULTADO_BITACORA} className={styles.list_card}>
                {TipoTabla == true ?
                <button
              ...
              <div className={styles.row}>
                ...
                <span className={styles.text}>{data.PLANTILLA_RESULTADO}</span>
              </div>
              ...
              <div className={styles.row}>
                <span className={styles.list_title}>Usuario que creo el seguimiento:</span>
                <span className={styles.text}>{data.USUARIO_CREADOR_RESULTADO}</span>
              </div>

              <div className={styles.row}>
                <span className={styles.list_title}>Fecha de creacion de resultado:</span>
                <span className={styles.text}>{data.FECHA_CREACION_RESULTADO_FORMAT}</span>
              </div>
