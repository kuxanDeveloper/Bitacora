...
function ListPrueba({ListPruebas, setListPruebas,InforOptionsSelc,InforPruebaXGrupo}) {
  ...
  useEffect(() => {
        ...
        InforPruebaXGrupo.length > 0)
    {
        ...
        setListPruebas((prevArray) => [...prevArray, data.COD_PRUEBA.toString()]);
  ...
  function DeleteRowEstatus(idRow) {
    setListPruebasNombre(ListPruebasNombre.filter((item) => item.codigo !== idRow));
        ...
        {InforOptionsSelc != null &&
                InforOptionsSelc != undefined
                  ? InforOptionsSelc.map((data, index) => (
                        ...
                        <option key={index} value={data.COD_PRUEBA}>{data.NOMBRE_PRUEBA}</option>
