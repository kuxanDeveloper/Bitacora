...
export const onSubmitCreateSitioAnat = ({
  DESCRIPCION,
  ESTADO
}) => {  
  return userService.CreateSitioAnatomico(
    DESCRIPCION,
    ESTADO
  ).then(() =>
  {
...
export const onSubmitEditSitioAnat = ({
  ID,
  DESCRIPCION,
  ESTADO
}) => {
  return userService.EditSitioAnatomico(
    ID,
  DESCRIPCION,
  ESTADO
  ).then(() =>
...
export const GetlistSitiosAnatomicos = (cookie,
    ID,ESTADO) => {
    return userService.GetlistSitiosAnatomicos(
        cookie,
        (ID == null ? "" : ID),
        (ESTADO == null ? "" : ESTADO)
    ).catch((error) => {
        ...
        if (
          ...
          console.log(error, "error al obtener la informacion de sitios anatomicos");
    });
